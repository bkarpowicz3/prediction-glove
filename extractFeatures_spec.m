function features = extractFeatures_spec(ecog, fs)
...
%         avgfreq = mean(P1(ind));
        [s, ~, ~] = spectrogram(x, [], [], bands(1, 1):bands(1, 2), fs);
    ...
    function avgfreq = findFreq2(x)
        ...
        [s, ~, ~] = spectrogram(x, [], [], bands(2, 1):bands(2, 2), fs);
        avgfreq = mean(mean(abs(s)));
    end
...
%         avgfreq = mean(P1(ind));
        ...
        [s, ~, ~] = spectrogram(x, [], [], bands(3, 1):bands(3, 2), fs);
        avgfreq = mean(mean(abs(s)));
    end
...
%         avgfreq = mean(P1(ind));
        ...
        [s, ~, ~] = spectrogram(x, [], [], bands(4, 1):bands(4, 2), fs);
        avgfreq = mean(mean(abs(s)));
    end
...
%         avgfreq = mean(P1(ind));
        ...
        [s, ~, ~] = spectrogram(x, [], [], bands(5, 1):bands(5, 2), fs);
        avgfreq = mean(mean(abs(s)));
    end
